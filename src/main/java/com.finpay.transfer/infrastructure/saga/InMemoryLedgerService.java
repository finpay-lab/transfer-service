 ...
 *
 * <p>Postings are immutable and keyed by {@code (transferId, leg)} — a
 * crash-recovery redrive cannot double-post. {@link #markAccountClosed(UUID)}
 * lets a test simulate a business failure at the CREDIT step.
 */
@Component
public class InMemoryLedgerService implements LedgerPostingPort {
    ...
    public void credit(UUID transferId, UUID accountId, BigDecimal amount, String currency) {
        if (closedAccounts.contains(accountId)) {
            throw new IllegalStateException("Destination account " + accountId + " is closed");
        }
